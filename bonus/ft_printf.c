...
/*   Updated: 2024/01/25 23:25:58 by sbakhit          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	formats_fun(const char **format, va_list args)
{
	...
	if (**format == 'c')
		putchar_counter(args, &counter, &result);
	if (**format == 's')
	...
	if (**format == '%')
		precentage_format(&counter, &result);
	return (counter);
}
...
// int	main(void)
// {
// 	printf("%d", ft_printf(" %04x ", 16));
// 	printf("\n");
// 	printf("%d", printf(" %04x ", 16));
// }
